...
const DEFAULT_BUILDING_IDS = ['castle', 'barracks', 'gold_mine', 'farm', 'crystal_core', 'watchtower'];
...
export function createNewPlayerProfile(now: string = new Date().toISOString()): PlayerProfile {
    return {
...
export function normalizePlayerProfile(value: unknown, now: string = new Date().toISOString()): PlayerProfile {
    if (!isRecord(value)) {
...
function normalizeResources(value: unknown, legacyGold: number): ResourceWallet {
    if (!isRecord(value)) {
        return {
            gold: legacyGold,
            food: 360,
