...
export class BattleScene extends Phaser.Scene {
  ...
  create() {
    ...
    const frameConfig = Array.from({ length: 15 }, (_, i) => ({
      key: `spn${i + 1}`,
    }));
    this.anims.create({
      key: "knight_run_anim",
    ...
    // 6. Cards
    this.createKnightCard(
      70,
      520,
      { id: "k1", name: "SLASH", damage: 25 },
      0x00ff00,
    );
    this.createKnightCard(
      180,
      520,
      { id: "k2", name: "CHARGE", damage: 25 },
      0x00ffff,
    );
    this.createKnightCard(
      290,
      520,
      { id: "k3", name: "CRITICAL", damage: 50 },
      0xff00ff,
    );
  }
  ...
  private createKnightCard(
    ...
    const bg = this.add
      .rectangle(0, 0, 90, 130, 0x1a1a1a)
      .setStrokeStyle(2, color);
    const nameTxt = this.add
      .text(0, -40, data.name, { fontSize: "12px", color: "#fff" })
      .setOrigin(0.5);
