...
export class CharacterSelectScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.createText(
      width / 2,
      76,
      "Your progress will stay linked to this profile.",
      {
        fontSize: "10px",
        color: "#94a3b8",
        align: "center",
    ...
    characters.forEach((char, index) => {
      this.createCharacterCard(
        char.id,
        char.name,
        charImages[index] ?? "chrono_knight",
        26,
        126 + index * 130,
        profile.level,
      );
  ...
  private createCharacterCard(
    ...
    const portraitFrame = this.add
      .circle(57, 52, 34, 0x0f172a, 0.96)
      .setStrokeStyle(1, isSelected ? 0x67e8f9 : 0x475569, 0.9);
    ...
    const desc = this.createText(
      108,
      51,
      isSelected ? `Saved at level ${level}` : "Tap to choose this commander",
      {
        fontSize: "10px",
        color: "#94a3b8",
      },
    ...
    card.on("pointerover", () =>
      this.tweens.add({ targets: card, x: x + 3, duration: 110 }),
    );
    card.on("pointerout", () =>
      this.tweens.add({ targets: card, x, duration: 110 }),
    );
  }
  ...
  private createButton(x: number, y: number, label: string) {
    ...
    const bg = this.add
      .rectangle(0, 0, 238, 42, 0x0e7490, 0.96)
      .setStrokeStyle(1, 0x67e8f9, 0.86);
