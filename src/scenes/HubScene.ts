...
export class HubScene extends Phaser.Scene {
  ...
  private createTopBar(profile: PlayerProfile, commanderName: string) {
    ...
    this.createText(
      20,
      42,
      `${commanderName} - Power ${this.getKingdomPower(profile)}`,
      {
        fontSize: "10px",
        color: "#93c5fd",
        fontStyle: "bold",
  ...
  private createBaseMap(profile: PlayerProfile) {
    ...
    this.createText(
      180,
      508,
      "Tap a building to upgrade, collect, or train troops.",
      {
        fontSize: "9px",
        color: "#94a3b8",
        align: "center",
  ...
  private createBuildingNode(
    ...
    const badgeBg = this.add
      .circle(0, 0, 13, 0x020617, 0.92)
      .setStrokeStyle(1, definition.accentColor, 0.95);
    ...
    node.on("pointerover", () =>
      this.tweens.add({ targets: node, y: y - 4, duration: 100 }),
    );
    node.on("pointerout", () =>
      this.tweens.add({ targets: node, y, duration: 100 }),
    );
  }
  ...
  private createBottomNav(profile: PlayerProfile) {
    ...
    this.createNavButton(60, height - 64, "WORLD", "March", 0x38bdf8, () =>
      this.showWorldPanel(),
    );
    this.createNavButton(
      ...
      height - 64,
      "ARMY",
      `${this.getArmySize(profile.army)} troops`,
      0xf97316,
      () => this.showArmyPanel(),
    );
    this.createNavButton(
      ...
      height - 64,
      "HEROES",
      `${profile.unlockedCardIds.length} cards`,
      0xa78bfa,
      () => this.showHeroPanel(),
    );
    this.createNavButton(300, height - 64, "STORE", "Soon", 0x22c55e, () =>
      this.showToast("Store will unlock with accounts."),
    );
  }
  ...
  private createNavButton(
    ...
    const bg = this.add
      .rectangle(0, 0, 68, 48, 0x0f172a, 0.96)
  ...
  private showBuildingPanel(buildingId: BuildingId) {
    ...
    const role = this.createText(
      0,
      -148,
      `${definition.role} - Level ${building.level}`,
      {
        fontSize: "10px",
        color: "#93c5fd",
        align: "center",
    ...
    const power = this.createStatPill(
      -75,
      -62,
      "POWER",
      String(getBuildingPower(definition, building.level)),
      definition.accentColor,
    );
    const nextPower = this.createStatPill(
      75,
      -62,
      "NEXT",
      String(getBuildingPower(definition, building.level + 1)),
      0x22c55e,
    );
    ...
    const upgradeButton = this.createPanelButton(
      0,
      70,
      canUpgrade ? "UPGRADE BUILDING" : "NEED RESOURCES",
    ...
    upgradeButton.on("pointerdown", () => {
      ...
      updateProfileInRegistry(this, (currentProfile) => ({
        ...
        buildings: currentProfile.buildings.map((item) =>
    ...
    const secondaryAction = this.createBuildingActionButton(
      definition,
      building,
      profile,
    );
  ...
  private createBuildingActionButton(
    definition: BuildingDefinition,
    building: BuildingState,
    ...
    if (definition.id === "barracks") {
      ...
      const button = this.createPanelButton(
        0,
        118,
        canTrain ? "TRAIN INFANTRY" : "NEED FOOD",
        canTrain ? 0xf97316 : 0x475569,
      );
    ...
    if (
      definition.id === "gold_mine" ||
      definition.id === "farm" ||
      definition.id === "crystal_core"
    ) {
      const button = this.createPanelButton(
        0,
        118,
      ...
      button.on("pointerdown", () => {
        updateProfileInRegistry(this, (currentProfile) => {
          const currentBuilding = this.getBuildingState(
            currentProfile,
            definition.id,
          );
          const gain = this.getCollectionGain(
            definition.id,
  ...
  private showWorldPanel() {
    ...
    panel.add(
      this.createText(
        0,
        -168,
        "Choose a march target for your commander army.",
        {
          fontSize: "10px",
          color: "#93c5fd",
          align: "center",
  ...
  private createWorldTarget(
    ...
    bg.lineStyle(
      1,
      isLocked ? 0x475569 : map.accentColor,
      isLocked ? 0.35 : 0.75,
    );
    bg.strokeRoundedRect(0, 0, 284, 76, 12);

    const marker = this.add
      .circle(24, 38, 15, isLocked ? 0x475569 : map.accentColor, 0.28)
    ...
    const reward = this.createText(
      50,
      52,
      `Reward ${map.rewards.xp} XP - ${map.rewards.gold} Gold - ${map.rewards.food ?? 0} Food`,
      {
        fontSize: "8px",
        color: "#cbd5e1",
      },
  ...
  private showArmyPanel() {
    ...
    panel.add(
      this.createText(0, -176, "ARMY COMMAND", {
        fontSize: "21px",
        color: "#ffffff",
        fontStyle: "bold",
        align: "center",
    ...
    panel.add(
      this.createText(
        0,
        -148,
        `Total troops ${this.getArmySize(profile.army)}`,
        {
          fontSize: "10px",
          color: "#fed7aa",
          align: "center",
    ...
    panel.add(
      this.createArmyRow(
        ...
        -98,
        "Infantry",
        profile.army.infantry,
        0xf97316,
      ),
    );
    panel.add(
      this.createArmyRow(-108, -50, "Archers", profile.army.archers, 0x22c55e),
    );
    panel.add(
      this.createArmyRow(-108, -2, "Mages", profile.army.mages, 0xa78bfa),
    );
    ...
    const trainButton = this.createPanelButton(
      0,
      130,
      canTrain ? `TRAIN ${barracks.level * 12} INFANTRY` : "NEED RESOURCES",
      canTrain ? 0xf97316 : 0x475569,
    );
  ...
  private createArmyRow(
    ...
    const bg = this.add
      .rectangle(108, 0, 216, 36, 0x0f172a, 0.92)
  ...
  private showHeroPanel() {
    ...
    panel.add(
      this.createText(0, -168, "Commander cards used in hero combat.", {
        fontSize: "10px",
        color: "#c4b5fd",
        align: "center",
      }).setOrigin(0.5),
  ...
  private createVaultCard(
    ...
    const power = this.createText(
      0,
      31,
      isOwned ? `POWER ${card.power}` : "LOCKED",
      {
        fontSize: "10px",
  ...
  private createCostRow(
    ...
    items.forEach((item, index) => {
      ...
      const value = this.createText(
        itemX,
        8,
        this.formatNumber(cost[item.key]),
        {
          fontSize: "10px",
  ...
  private createStatPill(
    ...
    const bg = this.add
      .rectangle(0, 0, 118, 44, 0x0f172a, 0.92)
      .setStrokeStyle(1, color, 0.55);
  ...
  private showToast(message: string) {
    const toast = this.add
      .container(this.scale.width / 2, 120)
  ...
  private getBuildingState(
    ...
    return (
      profile.buildings.find((building) => building.id === buildingId) ?? {
        id: buildingId,
  ...
  private getKingdomPower(profile: PlayerProfile): number {
    ...
    return (
      buildingPower + this.getArmySize(profile.army) * 4 + profile.level * 85
    );
  }
