...
import { getProfileFromRegistry, grantProfileReward } from '../progression/ProfileRegistry';
...
export class ResultScene extends Phaser.Scene {
    ...
    create(data: ResultSceneData = {}) {
        ...
        this.createRewardTile(48, 205, 'XP', `+${rewardResult?.reward.xp ?? 0}`, 0x38bdf8);
        this.createRewardTile(188, 205, 'GOLD', `+${rewardResult?.reward.gold ?? 0}`, 0xfacc15);
        this.createRewardTile(48, 278, 'FOOD', `+${rewardResult?.reward.food ?? 0}`, 0x22c55e);
        this.createRewardTile(188, 278, 'CRYSTAL', `+${rewardResult?.reward.crystal ?? 0}`, 0xa78bfa);
        ...
        this.add.rectangle(48, 443, 264 * xpRatio, 10, 0x22c55e, 1).setOrigin(0, 0.5);
        this.createText(48, 459, `${profile.xp}/${profile.xpToNextLevel} XP TO NEXT LEVEL`, {
            fontSize: '9px',
            color: '#94a3b8'
        });

        if (rewardResult && rewardResult.levelsGained > 0) {
            this.createText(width / 2, 514, `LEVEL UP +${rewardResult.levelsGained}`, {
                fontSize: '18px',
                color: '#86efac',
                fontStyle: 'bold',
                align: 'center',
            ...
            this.createText(width / 2, 514, data.victory ? 'Kingdom progress saved' : 'No rewards earned', {
                fontSize: '12px',
                color: '#94a3b8',
                align: 'center'
            }).setOrigin(0.5);
    ...
    private createRewardTile(x: number, y: number, label: string, value: string, color: number) {
        const bg = this.add.rectangle(x + 56, y, 112, 52, 0x0f172a, 0.92).setStrokeStyle(1, color, 0.42);
        const glow = this.add.circle(x + 18, y, 11, color, 0.18).setStrokeStyle(1, color, 0.78);
    ...
    private createContinueButton(x: number, y: number) {
        ...
        const bg = this.add.rectangle(0, 0, 214, 48, 0x0e7490, 0.96).setStrokeStyle(1, 0x67e8f9, 0.9);
        ...
        button.on('pointerover', () => this.tweens.add({ targets: button, scale: 1.035, duration: 110 }));
        button.on('pointerout', () => this.tweens.add({ targets: button, scale: 1, duration: 110 }));
