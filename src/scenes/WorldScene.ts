...
export class WorldScene extends Phaser.Scene {
    ...
    create() {
        ...
        const runFrames = [];
        for (let i = 1; i <= 15; i++) { runFrames.push({ key: `spn${i}` }); }
        ...
        const bg = this.add.image(width / 2, height / 2, 'login_bg').setDisplaySize(width, height);
        const guestBtn = this.add.container(width / 2, height * 0.85);
        const btnRect = this.add.rectangle(0, 0, 240, 50, 0x000000, 0.6).setStrokeStyle(2, 0x9400d3);
        const btnText = this.add.text(0, 0, 'PLAY AS GUEST', { fontSize: '18px', fontStyle: 'bold', color: '#ffffff' }).setOrigin(0.5);
        guestBtn.add([btnRect, btnText]);
        guestBtn.setSize(240, 50).setInteractive({ useHandCursor: true });
        guestBtn.on('pointerdown', () => {
            this.tweens.add({
                targets: this.cameras.main, zoom: 2.5, duration: 1500, ease: 'Cubic.easeInOut',
                onComplete: () => { loginContainer.destroy(); this.showCharacterSelection(); }
            });
        ...
        this.add.image(width / 2, height * 0.75, 'cave_floor').setScale(0.8).setDepth(-1).setAlpha(0.6);

        this.knight = this.add.container(width / 2, height * 0.7).setAlpha(0);
        const sprite = this.add.sprite(0, 0, 'spn1').setScale(0.25);
        const nameTag = this.add.text(0, -60, 'HERO', { fontSize: '14px', color: '#00ff00', fontStyle: 'bold' }).setOrigin(0.5);
    ...
    private showCharacterSelection() {
        ...
        const caveBg = this.add.rectangle(width/2, height/2, width, height, 0x050505);
        ...
        const name = this.add.text(0, 150, 'CHRONO KNIGHT', { fontSize: '22px', fontStyle: 'bold', color: '#ff4500' }).setOrigin(0.5);
    ...
    private createDashUI() {
        ...
        const cardFrame = this.add.rectangle(0, 0, 80, 110).setStrokeStyle(2, 0xff4500);
    ...
    update() {
        ...
        if (this.cursors.left.isDown) { vx = -speed; sprite.setFlipX(true); }
        else if (this.cursors.right.isDown) { vx = speed; sprite.setFlipX(false); }
        if (this.cursors.up.isDown) { vy = -speed * 0.7; }
        else if (this.cursors.down.isDown) { vy = speed * 0.7; }

        body.setVelocity(vx, vy);

        if (vx !== 0 || vy !== 0) {
            // Loop the regular walk animation.
            if (!sprite.anims.isPlaying || sprite.anims.currentAnim?.key !== 'knight_run_fast') {
    ...
    private createFogLayer() {
        for (let i = 0; i < 5; i++) {
            const fog = this.add.ellipse(Phaser.Math.Between(0, 360), Phaser.Math.Between(400, 640), 300, 100, 0x442266, 0.1);
            this.tweens.add({ targets: fog, alpha: 0.3, duration: 2000 + i * 500, yoyo: true, repeat: -1 });
        }
    }
}
